...
#### Rolling update strategy for web and worker deployments

When using topology spread constraints with `whenUnsatisfiable: DoNotSchedule`, the default Kubernetes rolling update strategy (`maxSurge: 25%`, `maxUnavailable: 0`) can cause deployment deadlocks. This happens because `maxSurge > 0` attempts to create new pods before terminating old ones, but `DoNotSchedule` prevents scheduling when all nodes are occupied.

To prevent this issue, the chart defaults to:
...
This configuration terminates one pod before creating its replacement, ensuring a node becomes available for the new pod.

**Configuration examples:**

```yaml
# Default (recommended for topology constraints with DoNotSchedule)
web:
  strategy:
    rollingUpdate:
      maxUnavailable: 1
...
# Alternative: allow surge if not using strict topology constraints
web:
  strategy:
    rollingUpdate:
      maxUnavailable: 0
...
# Conservative: update one pod at a time with no surge
web:
  strategy:
    rollingUpdate:
      maxUnavailable: 1
...
- **DoNotSchedule + maxSurge > 0**: Causes deadlocks when all nodes are occupied. The scheduler cannot place new pods until old ones terminate, but the rolling update waits for new pods to be ready before terminating old ones.
- **ScheduleAnyway + any strategy**: Works with any rolling update configuration since pods can be scheduled even when constraints are violated.
- **Availability during updates**: With `maxUnavailable: 1` and `maxSurge: 0`, capacity temporarily drops by one pod during updates. Ensure your replica count and resource allocation can handle this reduction.
